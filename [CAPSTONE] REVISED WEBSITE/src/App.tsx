  ...
  const renderPage = () => {
    switch (currentPage) {
      case 'jobs':
        return <JobPortal />;
      case 'jobsforyou':
        return <JobsForYou onApply={handleApplyToJob} onSaveJob={handleSaveJob} savedJobIds={savedJobs.map(job => parseInt(job.id))} />;
      case 'resume':
        return <ResumeBuilder />;
      case 'dashboard':
        return <ApplicantDashboard isLoggedIn={isLoggedIn} />;
      case 'about':
        return <AboutUs />;
      case 'applications':
